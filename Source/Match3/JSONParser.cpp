...
std::queue<JSON::Token> JSONParser::Tokenize(std::stringstream& ss)
{
	...
	while (!ss.eof())
	{
		...
		if (isdigit(ss.peek()) || ss.peek() == '-') {
			...
			if (std::all_of(value.begin() + (isNegative ? 1 : 0), value.end(), predicate) && value.back() != '.')
			{
				invalidChar = false;
...
void JSONParser::CheckTokenType(JSON::TokenType tokenType, std::initializer_list<JSON::TokenType> expectedTokens)
{
	auto predicate = [&tokenType](JSON::TokenType expectedType) { return tokenType == expectedType; };
	try
	{
		if (!std::any_of(expectedTokens.begin(), expectedTokens.end(), predicate))
		{
			std::string expectedTokensMsg;
			std::for_each(expectedTokens.begin(), expectedTokens.end(), [&expectedTokensMsg, &tokenType](JSON::TokenType type)
			{
				expectedTokensMsg += JSON::TypeToString(type) + ", ";
			});


			std::string msg = "Neispravan token! Ocekivani: " + expectedTokensMsg + "dobiven: " + JSON::TypeToString(tokenType);
